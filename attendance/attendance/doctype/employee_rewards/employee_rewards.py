...
class EmployeeRewards(Document):
    ...
    def on_submit(self):
        if self.action_type == 'Additional Salary' :
            self.submit_additional_salary()
        if self.action_type == 'Cash Rewards' :
            self.create_cash_reward()
    # def on_cancel (self):
    ...
    @frappe.whitelist()
    def create_cash_reward(self):
        if not self.amount:
            return
        reward_type = frappe.get_doc("Employee Reward Accounts" , self.employee_reward_accounts)
        doc = frappe.new_doc("Cash Rewards")
        doc.posting_date = self.payroll_date
        doc.naming_series = "CASH-RWRDS-.####"
        doc.employee = self.employee
        doc.employee_name = self.employee_name
        doc.company = self.company
        doc.department = self.department
        doc.employee_rewards = self.name


        doc.amount = self.amount

        doc.reward_type = self.employee_reward_accounts
        doc.expense_account = reward_type.expense_account
        doc.payment_account = reward_type.payment_account
        doc.employee_reward_account = reward_type.employee_reward_account
        doc.earning_additional_salary = reward_type.earning_additional_salary
        doc.deduction_additional_salary = reward_type.deduction_additional_salary
        ...
        # self.ref_doctype = doc.doctype
        # self.ref_docname = doc.name
        self.cash_rewards = doc.name
        # self.db_set('ref_doctype',self.ref_doctype)
        # self.db_set('ref_docname',self.ref_docname)
        self.db_set('cash_rewards',self.cash_rewards)
