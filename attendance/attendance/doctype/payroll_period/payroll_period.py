...
from erpnext.setup.utils import get_holiday_dates_for_employee
...
def get_payroll_period_days(start_date, end_date, employee, company=None):
    ...
    if len(payroll_period) > 0:
        actual_no_of_days = date_diff(
            getdate(payroll_period[0][2]), getdate(payroll_period[0][1])) + 1
        ...
        if not cint(
            ...
            holidays = get_holiday_dates_for_employee(
                employee, getdate(payroll_period[0][1]), getdate(
                    payroll_period[0][2])
            )
    ...
    if relieving_date and getdate(relieving_date) < getdate(period_end):
        ...
        if month_diff(period_end, start_date) > 1:
            start_date = add_months(
                start_date, -(month_diff(period_end, start_date) + 1))

    total_sub_periods, remaining_sub_periods = 0.0, 0.0

    if payroll_frequency == "Monthly" and not depends_on_payment_days:
        total_sub_periods = month_diff(
            payroll_period.end_date, payroll_period.start_date)
        ...
        remaining_days_in_payroll_period = date_diff(
            period_end, start_date) + 1
        remaining_sub_periods = flt(
            remaining_days_in_payroll_period) / flt(salary_days)
